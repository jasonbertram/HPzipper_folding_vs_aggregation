...
plt.figure()
#structure_history[:,:-1]=structure_history[:,:-1]/(len(sequence_history[0])*10)
plt.plot(structure_history[:,:])
plt.plot(np.sum(structure_history[:,:],1))
plt.plot(local_peaking_times,len(local_peaking_times)*[0],'o')
plt.ylabel(r"F,A")
plt.xlabel(r"Substitution")
plt.savefig("/N/dc2/scratch/jxb/HPzipper/fitness.pdf")

plt.figure()
plt.plot(structure_history[:,-1])
